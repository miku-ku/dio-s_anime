...
2. **用户系统**：注册、登录（记住我）、个人主页（左侧改头像/改密码，中间弹窗编辑个人简介）。

> ⚠️ 资源来自第三方聚合接口，多为 BT/磁力下载（**不是在线播放**），且涉及版权灰色地带，
> 仅作学习演示，请勿公开运营。
>
> 数据源说明：番剧封面/元数据用 **AniList**（Bangumi 在国内需翻墙，故弃用）；
> 磁力资源用 **AnimeGarden** 开放 API。
...
└── frontend/               # 前端静态页面（由 FastAPI 托管）
    ├── search.html         # 动漫搜索页：番剧卡片网格（海报）+ 类型筛选 + 中文兜底直搜
    ├── anime.html          # 番剧详情：封面/信息 + 该番全部磁力链整合一页
    ├── index.html          # 个人主页：左栏设置（头像/改密）+ 中间简介（弹窗编辑）
    ...
    └── js/
        ...
        ├── search.js / anime.js / index.js / login.js / register.js
```
...
4. 首页加载时调用 `GET /api/me`，后端校验令牌并返回当前用户信息
